// ==UserScript==
// @name          SEU研究生讲座抢课脚本 v2.5 (原生Cookie版) - 增强防反爬
// @namespace     http://tampermonkey.net/
// @version       2.5
// @description   完全使用浏览器原生Cookie和Session发送请求，增强了HTTP Header和请求延迟，解决倒计时后会话失效问题。
...
    // *** 版本更新到 2.5 并修改描述 ***
    console.log("✅ SEU Grab Script v2.5 (Native Cookie Version with Anti-Bot Fixes) is Running!");
...
    /**
     * 【修正】使用原生 fetch 和浏览器 Cookie - 自动处理会话，并添加关键 Headers 模仿 AJAX
     */
    async function fetchRequest(url, options = {}, timeout = 10000) {
        const controller = new AbortController();
        const timeoutId = setTimeout(() => controller.abort(), timeout);

        // 【修正】添加默认 Headers，模仿浏览器 AJAX 请求
        const defaultHeaders = {
            'User-Agent': navigator.userAgent,
            'Referer': BASE_URL + 'm/home', // 模仿从应用首页发起
...
                method: 'POST',
                // Content-Type 必须保持精确，用于 POST body 的正确解析
...
                // 检查是否包含 Nginx 或通用错误信息，视为服务器过载或临时故障
...
                <h3 style="margin-top: 0; color: #4CAF50;">🎓 SEU 抢课助手 v2.5 (原生Cookie版) - 增强防反爬</h3>
